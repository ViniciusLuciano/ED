...
typedef struct cidade {
    ...
    TabelaHash tabelaHashVerticesG;
...
Cidade criarCidade(int i, int nq, int nh, int ns, int nr, int np, int nm) {
    ...
    for (int i = 0; i < 11; i++) cidade->registradores[i] = NULL;
    ...
    cidade->tabelaHashVerticesG = criarTabelaHash(1000, VerticeG_equals, VerticeG_getChave, NULL);
    ...
    cidade->arvoreVerticesG = criarArvore(VerticeG_compararChave, destruirVerticeG);
    return cidade;
}

void Cidade_setGrafo(Cidade c, Grafo grafo) {
    pCidade cidade = (pCidade)c;
    cidade->grafo = grafo;
}
...
void destruirCidade(Cidade c) {
    ...
    for (int i = 0; i < 11; i++) {
        Ponto p = cidade->registradores[i];
        if (p != NULL) destruirPonto(p);
    }
    ...
    destruirTabelaHash(cidade->tabelaHashVerticesG);
    ...
    destruirArvore(cidade->arvoreVerticesG);
    destruirGrafo(cidade->grafo);
    destruirTNULL();
    free(cidade);
}

void Cidade_setRegistrador(Cidade c, int index, double x, double y) {
    pCidade cidade = (pCidade) c;
    Ponto pontoAnt = cidade->registradores[index];
    cidade->registradores[index] = criarPonto(x, y);
    if (pontoAnt != NULL) {
        destruirPonto(pontoAnt);
    }
}

Ponto Cidade_getRegistrador(Cidade c, int index) {
    pCidade cidade = (pCidade) c;
    return cidade->registradores[index];
}

void Cidade_setVerticeG(Cidade c, VerticeG verticeG) {
    pCidade cidade = (pCidade) c;
    Arvore_inserir(cidade->arvoreVerticesG, verticeG);
    TabelaHash_inserir(cidade->tabelaHashVerticesG, verticeG);
}

Arvore Cidade_getArvoreVerticesG(Cidade c) {
    pCidade cidade = (pCidade) c;
    return cidade->arvoreVerticesG;
}

VerticeG Cidade_getVerticeG(Cidade c, char *id) {
    pCidade cidade = (pCidade) c;
    return TabelaHash_getObjeto(cidade->tabelaHashVerticesG, id);
}
...
void Cidade_processarBombaRaioLuminoso(Cidade c, double x, double y, FILE *svg, bool brl, FILE* txt, FILE* arq) {
    ...
    if (!brl) {
        ...
        Node vertices = Arvore_getRaiz(cidade->arvoreVerticesG);
        forEach(vertice, vertices) {
            if (Node_getAux(vertice) == 0) {
                VerticeG v = Node_getObjeto(vertice);
                VerticeG_destruirArestasNoPoligono(v, polig);
            }
        }
...
void _Cidade_navegarGrafo(Cidade c, VerticeG* percurso) {
    pCidade cidade = (pCidade) c;

    char str[150], instrucao[10];
    VerticeG vertice = percurso[0];
    VerticeG destino;

    for (int i = 1; ; i++) {
        destino = percurso[i];
        if (Vertige_get_inicial(destino) == false) break;
    }


    while (true) {
        printf("- Navegar grafo\n");
        printf("- comando-> ");
        fgets(str, 150, stdin);
        sscanf(str, "%s", instrucao);

        if (strcmp(instrucao, "n") == 0) return;
        if (strcmp(instrucao, "n") == 0) {

            VerticeG aux = VerticeG_getPosic(vertice, 'N');
            if (aux != NULL) vertice = aux;
            else printf("Não há caminho para o Norte.\n");

        } else if (strcmp(instrucao, "s") == 0) {

            VerticeG aux = VerticeG_getPosic(vertice, 'S');
            if (aux != NULL) vertice = aux;
            else printf("Não há caminho para o Sul.\n");

        } else if (strcmp(instrucao, "l") == 0) {

            VerticeG aux = VerticeG_getPosic(vertice, 'L');
            if (aux != NULL) vertice = aux;
            else printf("Não há caminho para o Leste.\n");
            
        } else if (strcmp(instrucao, "o") == 0) {

            VerticeG aux = VerticeG_getPosic(vertice, 'O');
            if (aux != NULL) vertice = aux;
            else printf("Não há caminho para o Oeste.\n");
            
        } else if (strcmp(instrucao, "rr") == 0) {

            percurso = Dijkstra(cidade->grafo, vertice, destino, ArestaG_get_vm);
            vertice = percurso[0];

        } else if (strcmp(instrucao, "rc") == 0) {

            percurso = Dijkstra(cidade->grafo, vertice, destino, ArestaG_get_cmp);
            vertice = percurso[0];
        }
    }
}
...
/*
Qual o melhor trajeto entre a origem está
que no registrador r1 e o destino que está
no registrador r2. O percurso na
representação pictórica deve indicar o
trajeto mais curto na cor cormc e o trajeto
mais rápido com a cor cormr
*/
void Cidade_processarPercurso(Cidade c, int r1, int r2, char *cormc, char *cormr, FILE* svg, FILE* txt, bool modoInterativo, bool qr) {
    pCidade cidade = (pCidade) c;

    Ponto p1 = Cidade_getRegistrador(c, r1);
    Ponto p2 = Cidade_getRegistrador(c, r2);

    if (p1 == NULL || p2 == NULL) return;

    // Encontrar os verticeg mais proximos dos pontos p1 p2
    double menor_dist_p1 = INFINITY, menor_dist_p2 = INFINITY;
    double dist_p1, dist_p2;
    VerticeG v1, v2;
    Node vertices = Arvore_getRaiz(cidade->arvoreVerticesG);
    forEach(vertice, vertices) {
        if (Node_getAux(vertice) == 0) {
            ...
            dist_p1 = distanciaL2(VerticeG_get_x(v), VerticeG_get_y(v), Ponto_get_x(p1), Ponto_get_y(p1));
            dist_p2 = distanciaL2(VerticeG_get_x(v), VerticeG_get_y(v), Ponto_get_x(p2), Ponto_get_y(p2));
            if (dist_p1 < menor_dist_p1) {
                menor_dist_p1 = dist_p1;
                v1 = v;
            }

            if (dist_p2 < menor_dist_p2) {
                menor_dist_p2 = dist_p2;
                v2 = v;
            }
        }
    }
    ...
    if (modoInterativo) {
        if (qr) {
            ...
            VerticeG* percursomr = Dijkstra(cidade->grafo, v1, v2, ArestaG_get_tempo);
            if (percursomr == NULL) printf("Não há caminho ao destino.\n");
            else _Cidade_navegarGrafo(c, percursomr);
            free(percursomr);

        } else {

            VerticeG* percursomc = Dijkstra(cidade->grafo, v1, v2, ArestaG_get_cmp);
            if (percursomc == NULL) printf("Não há caminho ao destino.\n");
            else _Cidade_navegarGrafo(c, percursomc);
            free(percursomc);
        }
    } else {
        VerticeG* percursomc = Dijkstra(cidade->grafo, v1, v2, ArestaG_get_cmp);
        VerticeG* percursomr = Dijkstra(cidade->grafo, v1, v2, ArestaG_get_tempo);
        svg_escreverPercursos(svg, percursomc, percursomr, cormr, cormc, txt);
        free(percursomc);
        free(percursomr);
    }
}