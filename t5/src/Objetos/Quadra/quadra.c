...
void Quadra_calcularEnderecoMorador(Quadra q, Morador m, double *x, double *y) {
    pQuadra quadra = (pQuadra) q;
    *x = quadra->x;
    *y = quadra->y;

    if (Morador_get_face(m) == 'N') {
        *x += Morador_get_num(m);
        *y += quadra->h;
    } else if (Morador_get_face(m) == 'S') {
        *x += Morador_get_num(m);
    } else if (Morador_get_face(m) == 'L') {
        *y += Morador_get_num(m);
        *x += quadra->w;
    } else if (Morador_get_face(m) == 'O') {
        *y += Morador_get_num(m);
    }
}