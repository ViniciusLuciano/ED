...
int main(int argc, char* argv[]) {
    ...
    char *nomeArquivoVia = NULL;
    bool modoInterativo = false;

    bool argumentosCorretos = lerArgumentos(argc, argv, &dirEntrada, &nomeArquivoEntrada, 
                                    &nomeArquivoConsulta, &dirSaida, &nomeArquivoEC, &nomeArquivoPM, &modoInterativo, &nomeArquivoVia);
    ...
    if (nomeArquivoVia != NULL) {
        FILE *arquivoVia = abrirArquivo( dirEntrada, nomeArquivoVia, "r" );
        if (arquivoVia == NULL)
            return 1;
        processarArquivoVia(arquivoVia, &cidade);
        fclose(arquivoVia);
    }
    ...
    desalocarArgumentos(dirEntrada, nomeArquivoEntrada, nomeArquivoConsulta, dirSaida, nomeArquivoEC, nomeArquivoPM, nomeArquivoVia);
