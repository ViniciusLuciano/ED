...
void svg_escreverTriangulo(FILE *SVG, double x1, double y1, Vertice v2, Vertice v3, bool brl) {
	//double x1 = Vertice_get_x(v1), y1 = VerticeG_get_y(v1);
...
void svg_escreverLinha(FILE* svg, double x1, double y1, double x2, double y2, char *cor) {
...
void _escreverTxt(VerticeG verticeAntAnt, VerticeG verticeAnt, VerticeG vertice, FILE *txt) {
	if (VerticeG_get_x(verticeAntAnt) == VerticeG_get_x(verticeAnt) && VerticeG_get_x(verticeAnt) == VerticeG_get_x(vertice)
	|| VerticeG_get_y(verticeAntAnt) == VerticeG_get_y(verticeAnt) && VerticeG_get_y(verticeAnt) == VerticeG_get_y(vertice)) {
		fprintf(txt, "continue reto na rua 'RUA', ");

	} else if (VerticeG_get_x(verticeAntAnt) == VerticeG_get_x(verticeAnt) && VerticeG_get_x(verticeAnt) > VerticeG_get_x(vertice)
	|| VerticeG_get_y(verticeAntAnt) == VerticeG_get_y(verticeAnt) && VerticeG_get_y(verticeAnt) < VerticeG_get_y(vertice)) {
		fprintf(txt, "vire à esquerda na rua 'RUA', ");
	} else if (VerticeG_get_x(verticeAntAnt) == VerticeG_get_x(verticeAnt) && VerticeG_get_x(verticeAnt) < VerticeG_get_x(vertice)
	|| VerticeG_get_y(verticeAntAnt) == VerticeG_get_y(verticeAnt) && VerticeG_get_y(verticeAnt) > VerticeG_get_y(vertice)) {
		fprintf(txt, "vire à direita na rua 'RUA', ");
	}	
}

void svg_escreverPercursos(FILE* svg, VerticeG* percursomc, VerticeG* percursomr, char* cormr, char* cormc, FILE* txt) {
	VerticeG verticeAntAnt;

	if (percursomc != NULL) {

		for (int i = 1; ; i++) {
			VerticeG verticeAnt = percursomc[i-1];
			VerticeG vertice = percursomc[i];
			if (Vertige_get_inicial(vertice) == false) break;

			if (i == 1) {
				fprintf(txt, "Bom dia, inicie reto na rua 'RUA', ");
			} else {
				_escreverTxt(verticeAntAnt, verticeAnt, vertice, txt);
			}

			double x1 = VerticeG_get_x(verticeAnt), y1 = VerticeG_get_y(verticeAnt);
			double x2 = VerticeG_get_x(vertice), y2 = VerticeG_get_y(vertice);
			
			svg_escreverLinha(svg, x1, y1, x2, y2, cormc);
			verticeAntAnt = verticeAnt;
		}
	...
	if (percursomr != NULL) {
		for (int i = 1; ; i++) {
			VerticeG verticeAnt = percursomr[i-1];
			VerticeG vertice = percursomr[i];
			if (Vertige_get_inicial(vertice) == false) break;
			...
			svg_escreverLinha(svg, x1 - 7, y1 - 7, x2 - 7, y2 - 7, cormr);
			verticeAntAnt = verticeAnt;
		}
